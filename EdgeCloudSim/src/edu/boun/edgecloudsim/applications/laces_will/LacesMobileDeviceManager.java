 ...
 * LacesMobileDeviceManager sends tasks to the edge servers or
...
import edu.boun.edgecloudsim.applications.laces_will.LacesEdgeOrchestrator;
import edu.boun.edgecloudsim.applications.laces_will.WeightProvider;
...
public class LacesMobileDeviceManager extends MobileDeviceManager {

	private final Map<Integer, Double> taskStartTimes = new HashMap<>();
	private static final double FAILURE_REWARD = -3.0;
	private static final double LATENCY_NORMALIZER = 10.0; // seconds
	...
	public LacesMobileDeviceManager() throws Exception{
	}
	...
	/********************************* for RL **************************************/
	private void sendReward(double reward) {
		try {
			LacesEdgeOrchestrator orch =
					(LacesEdgeOrchestrator) SimManager.getInstance().getEdgeOrchestrator();

			WeightProvider provider = orch.getBridge().getWeightProvider();
			if (provider != null) {
				provider.reportReward(reward);
			} else {
				//SimLogger.printLine("Reward not sent: provider is null");
			}
		} catch (Exception e) {
			SimLogger.printLine("Failed to send reward to Python: " + e.getMessage());
		}
	}

	private void reportSuccessReward(Task task) {
		Double startTime = taskStartTimes.remove(task.getCloudletId());
		if (startTime == null) {
			sendReward(0.0);
			return;
		}

		double latency = CloudSim.clock() - startTime;
		double normalizedLatency = Math.min(latency / LATENCY_NORMALIZER, 1.0);
		double reward = 1.0 - normalizedLatency;

		// SimLogger.printLine("[RL] SUCCESS task=" + task.getCloudletId()
		// 		+ " latency=" + latency
		// 		+ " reward=" + reward);
	...
	private void reportFailureReward(Task task, String reason) {
		taskStartTimes.remove(task.getCloudletId());

		// SimLogger.printLine("[RL] FAIL task=" + task.getCloudletId()
		// 		+ " reason=" + reason
		// 		+ " reward=" + FAILURE_REWARD);

		sendReward(FAILURE_REWARD);
	}
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		if(task.getAssociatedDatacenterId() == SimSettings.CLOUD_DATACENTER_ID){
			...
			if(delay > 0)
			{
				...
				{
					SimLogger.getInstance().failedDueToMobility(task.getCloudletId(), CloudSim.clock());
					reportFailureReward(task, "edge_mobility");
				}
			}
			else
			{
				SimLogger.getInstance().failedDueToBandwidth(task.getCloudletId(), CloudSim.clock(), delayType);
				reportFailureReward(task, "edge_download_bandwidth");
			}
		}
	}
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		switch (ev.getTag()) {
			case UPDATE_MM1_QUEUE_MODEL:
			{
				...
				((LacesNetworkModel)networkModel).updateMM1QueeuModel();
			...
			case REQUEST_RECEIVED_BY_EDGE_DEVICE_TO_RELAY_NEIGHBOR:
			{
				...
				else
				{
					...
					reportFailureReward(task, "man_relay_bandwidth");
				}
	...
	public void submitTask(TaskProperty edgeTask) {
		...
		taskStartTimes.put(task.getCloudletId(), CloudSim.clock());
		...
		if(delay>0 || nextHopId == SimSettings.MOBILE_DATACENTER_ID){
			...
			if(selectedVM != null){
				...
				reportFailureReward(task, "vm_capacity");
			}
		}
		else
		{
			...
			reportFailureReward(task, "upload_bandwidth");
		}
	}
