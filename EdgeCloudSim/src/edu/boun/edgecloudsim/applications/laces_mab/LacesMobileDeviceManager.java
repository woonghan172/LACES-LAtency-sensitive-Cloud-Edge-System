...
public class LacesMobileDeviceManager extends MobileDeviceManager {
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		if(task.getAssociatedDatacenterId() == SimSettings.CLOUD_DATACENTER_ID){
			...
			if(WanDelay > 0)
			{
				...
				{
					recordBanditTaskFailure(task, CloudSim.clock(), "mobility");
					SimLogger.getInstance().failedDueToMobility(task.getCloudletId(), CloudSim.clock());
				}
			}
			else
			{
				recordBanditTaskFailure(task, CloudSim.clock(), "wan_bandwidth");
			...
			recordBanditTaskSuccess(task, CloudSim.clock());
			...
			{
				recordBanditTaskFailure(task, CloudSim.clock(), "edge_bandwidth");
		...
		switch (ev.getTag()) {
			...
			case REQUEST_RECEIVED_BY_EDGE_DEVICE_TO_RELAY_NEIGHBOR:
			{
				...
				else
				{
					...
					recordBanditTaskFailure(task, CloudSim.clock(), "man_relay_bandwidth");
			...
			case RESPONSE_RECEIVED_BY_EDGE_DEVICE_TO_RELAY_MOBILE_DEVICE:
			{
				...
				{
					recordBanditTaskFailure(task, CloudSim.clock(), "wlan_bandwidth");
	...
	public void submitTask(TaskProperty edgeTask) {
		...
		if(delay>0 || nextHopId == SimSettings.MOBILE_DATACENTER_ID){
			...
			if(selectedVM != null){
				...
				// VM capacity failure
				recordBanditTaskFailure(task, CloudSim.clock(), "vm_capacity");
		...
		else
		{
			// Bandwidth rejection
			if(nextHopId != SimSettings.MOBILE_DATACENTER_ID) {
				recordBanditTaskFailure(task, CloudSim.clock(), "uplink_bandwidth");
	...
	private void recordBanditTaskSuccess(Task task, double eventTime) {
		LacesEdgeOrchestrator orchestrator = getLacesBanditOrchestrator();
		if(orchestrator != null)
			orchestrator.recordTaskCompletion(task, eventTime);
	}

	private void recordBanditTaskFailure(Task task, double eventTime, String reason) {
		LacesEdgeOrchestrator orchestrator = getLacesBanditOrchestrator();
		if(orchestrator != null)
			orchestrator.recordTaskFailure(task, eventTime, reason);
	}

	private LacesEdgeOrchestrator getLacesBanditOrchestrator() {
		EdgeOrchestrator orchestrator = SimManager.getInstance().getEdgeOrchestrator();
		if(orchestrator instanceof LacesEdgeOrchestrator)
			return (LacesEdgeOrchestrator)orchestrator;
		return null;
	}
