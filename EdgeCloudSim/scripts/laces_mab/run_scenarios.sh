...
# Prepare per-process runner scripts in the current simulation output folder.
rm -f ${simulation_out_folder}/tmp_runner*.sh
for (( i=0; i<$num_of_processes; i++ ))
do
	echo "#!/bin/bash" > ${simulation_out_folder}/tmp_runner${i}.sh
	echo "set -e" >> ${simulation_out_folder}/tmp_runner${i}.sh
done
...
for (( i=0; i<$num_of_processes; i++ ))
do
	runner_script=${simulation_out_folder}/tmp_runner${i}.sh
	if grep -q "runner.sh" "$runner_script"; then
		chmod +x "$runner_script"
		setsid "$runner_script" &
		pid=$!
